...
- **worktree cache save**：`use_cache=true` 时 compile 非 skipped 即 save（含失败/取消）；save 前删除所有同类（`worktree-v3-*`）缓存条目
- **ccache save**：同上规则；save 前删除所有同类（`ccache-v3-*`）缓存条目；`CCACHE_MAXSIZE=3G`
